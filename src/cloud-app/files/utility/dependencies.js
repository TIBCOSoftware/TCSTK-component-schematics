...
const pkgJsonPath = '/packageORG.json';
...
function addPackageJsonDependency(tree, dependency) {
    ...
    if (!depsNode) {
        // Haven't found the dependencies key, add it to the root of the packageORG.json.
...
function _readPackageJson(tree) {
    ...
    if (buffer === null) {
        throw new schematics_1.SchematicsException('Could not read packageORG.json.');
    }
    const content = buffer.toString();
    const packageJson = core_1.parseJsonAst(content, core_1.JsonParseMode.Strict);
    if (packageJson.kind != 'object') {
        throw new schematics_1.SchematicsException('Invalid packageORG.json. Was expecting an object');
    }
    return packageJson;
}
